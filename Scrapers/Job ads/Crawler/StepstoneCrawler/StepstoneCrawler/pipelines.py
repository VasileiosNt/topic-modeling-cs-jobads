...
from StepstoneCrawler.models import create_table,db_connect,StepstoneJob
import StepstoneCrawler.settings


class StepstonecrawlerPipeline(object):
    def __init__(self):
        engine = db_connect()
        create_table(engine)
...
class CheckForDuplicatesPipeline(object):
    ...
    def process_item(self,item,spider):
        session = self.Session()
        item_exists = session.query(StepstoneJob).filter_by(JobId = item['JobId']).first()
