...
class IndeedSpider(scrapy.Spider):
    ...
    def __init__(self, filename=None):
        if filename:
            with open("urls.txt") as f:
                ...
                self.start_urls = [
                    url + str(i * 10) for i in range(0, 25) for url in self.start_urls
                ]
    ...
    def parse(self, response):
        ...
        for job in result:
            ...
            rating = job.css(
                ".sjcl .ratingsDisplay .ratingNumber .ratingsContent::text"
            ).get()
            if rating is None:
            ...
            yield response.follow(
                next_url, callback=self.parse_job, meta={"job_item": job_item}
            )
    ...
    def parse_job(self, response):
        ...
        for data in result:
            ...
            # addLocation
            loader.add_css(
                "Location", "span.jobsearch-JobMetadataHeader-iconLabel::text"
            )
            # addDescription
            loader.add_xpath(
                "Description", 'normalize-space(.//div[@id="jobDescriptionText"])'
            )
